...
for i1 = 1:length(datasetCandi)
    ...
    if ~exist(fname2, 'file')
        ...
        lambdas = 10.^(-5:1:5);
        paramCell = MKCAPA_MKKM_build_param(lambdas);
        nParam = length(paramCell);
        MKCAPA_MKKM_result = zeros(nParam, 1, nRepeat, 13);  
        MKCAPA_MKKM_time = zeros(nParam, 1);
        for iParam = 1:nParam
            disp(['MKCAPA_MKKM iParam= ', num2str(iParam), ', totalParam= ', num2str(nParam)]);
            fname3 = fullfile(prefix_mdcs, [data_name, '_12kAllFea_MKCAPA_MKKM_', num2str(iParam), '.mat']);
            if exist(fname3, 'file')
                ...
                for iRepeat = 1:nRepeat
                    ...
                    [Y_EXP, obj_value] = MKCAPA(Ks_cell, nCluster, param.lambda, NITER);
                   ...
                   MKCAPA_MKKM_result(iParam, 1, iRepeat, :) = [result_10'];
                end
                tt = toc(t1);
               MKCAPA_MKKM_time(iParam) = tt / nRepeat;
                save(fname3, 'MKCAPA_MKKM_result', 'tt', 'param');
            end
        end

        a1 = sum(MKCAPA_MKKM_result, 2);
        a2 = sum(a1, 3);
        a3 = reshape(a2, nParam, 13);
        a4 = a3 / nRepeat;
        MKCAPA_MKKM_result_summary = [max(a4, [], 1), sum(MKCAPA_MKKM_time) / nParam];
        save(fname2, 'MKCAPA_MKKM_result', 'MKCAPA_MKKM_time', 'MKCAPA_MKKM_result_summary');
